...
```{r}
if(!file.exists(paste0(cddir,"results.rds"))){
...
results <- map2_df(.x = validation_data$splits,
                           ...
                           ~grad_class(formula=ba_complete_formula,
                                       ...
                                       split = .x, 
                                       id = .y))
   save(results,file=paste0(cddir,"results.rds"))
} else {
   load(paste0(cddir,"results.rds"))
}
...
```{r}
pb<-progress_estimated(length(unique(results$id)))

roc_auc_error<-function(data,truth,prediction){
   tryCatch(
      {
      roc_auc(data,truth,prediction)
         },
        error=function(cnd) {
           return(NA)
        }
...
results <- map2_df(.x = results$splits,
                   .y = validation_data$id,
                   ~roc_auc_error(data=.,
                                  truth=truth,
...
results%>%
   ...
   summarize(AUC=roc_auc_error(data=.,truth=truth,prediction=.pred_Completed))->auc_summary

   roc_auc_error(data=results_sub,
                            truth=results_sub$truth,
                            prediction=results_sub$`.pred_Completed`)

results_sub%>%roc_auc(data=.,
                            truth=truth,
                            prediction=.pred_Completed)


results_sub<-filter(results,id=="Resample0002")      
   
results_sub<-filter(results,id=="Resample0001")   

results_sub%>%roc_auc_error
```

```{r}
gg<-ggplot(auc,aes(x=.metric))
